...
abstract class AuthAbstractClient
{
     ...
     * {@inheritdoc}
     */
    public function GetToken(array $ctx, \AutoNotes\Auth\LoginRequest $in): \AutoNotes\Auth\LoginResponse
    {
        $ctx = Context::withPackageName($ctx, 'xelbot.com.autonotes.auth');
        $ctx = Context::withServiceName($ctx, 'Auth');
        $ctx = Context::withMethodName($ctx, 'GetToken');

        $out = new \AutoNotes\Auth\LoginResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/xelbot.com.autonotes.auth.Auth/GetToken';
        } else {
            $url = $url.'/'.$this->prefix.'/xelbot.com.autonotes.auth.Auth/GetToken';
        }
     ...
     * {@inheritdoc}
     */
    public function RefreshToken(array $ctx, \AutoNotes\Auth\RefreshTokenRequest $in): \AutoNotes\Auth\LoginResponse
    {
        $ctx = Context::withPackageName($ctx, 'xelbot.com.autonotes.auth');
        $ctx = Context::withServiceName($ctx, 'Auth');
        ...
        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/xelbot.com.autonotes.auth.Auth/RefreshToken';
        } else {
            $url = $url.'/'.$this->prefix.'/xelbot.com.autonotes.auth.Auth/RefreshToken';
        }
