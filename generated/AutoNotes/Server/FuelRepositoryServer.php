 ...
 */
final class FuelRepositoryServer implements
    ...
    ServerWithPathPrefix
{
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch ($method) {
            ...
            case 'FindFuel':
                return $this->handleFindFuel($ctx, $req);
    ...
    private function handleFindFuel(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleFindFuelJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleFindFuelProtobuf($ctx, $req);
                break;
    ...
    private function handleFindFuelJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $out = $this->svc->FindFuel($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling FindFuel. null responses are not supported'));
            }
    ...
    private function handleFindFuelProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'FindFuel');
